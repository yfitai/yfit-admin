...
function interpolateJoints(frameA: Record<string, number[]>, frameB: Record<string, number[]>, t: number): Record<string, number[]> {
...
export default function FormAnalysisShowcase() {
  ...
  const [repHistory, setRepHistory] = useState<Array<{type: string; message: string}>>([]);
  const [paused, setPaused] = useState(false);
  const stateRef = useRef<{frameIdx: number; t: number; repCount: number; repHistory: Array<{type: string; message: string}>; paused: boolean}>({ frameIdx: 0, t: 0, repCount: 0, repHistory: [], paused: false });

  const exercise = ALL_EXERCISES[exerciseIdx];

  const drawFrame = useCallback((canvas: HTMLCanvasElement, joints: Record<string, number[]>, feedbackColor: string) => {
    ...
    for (const [name, pos] of Object.entries(joints)) {
      const x = (pos as number[])[0] * w;
      const y = (pos as number[])[1] * h;
